...
#data will be 10-day, 20-day, 50-day, 100-day and 200-day average. 
#so if n is 8, i will be asking from today what will the price be 8 days from now (current + n)

#program does not handle shifting date entries for a given n value. I.e if I train with 
# 10, 200 MA to predict value n days into the future, and then I train with 10, 100 MA to predict 
# n days into the future, there will be an error. use tablemodify2.py for that. It changes up the table
#based on your new date span. 
...
name = booleanchops.boolean_table(conn, n, dates)
...
print(entries_in_numpy)
predicted = clf2.predict(numpy_data[split:])
predicted2 = str(predicted)

results = numpy_target[split:]
